...
export default function CoursePlayerPage({ params }) {
    ...
    return (
        <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-white rounded-2xl shadow-sm border border-gray-200">
            {/* Sidebar - Syllabus */}
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {/* Header */}
                <div className="h-16 border-b border-gray-200 flex items-center justify-between px-8 bg-white flex-shrink-0">
                    <div>
                        <span className="text-sm text-gray-500">Current Topic</span>
                        <h2 className="text-lg font-bold text-gray-900">{activeTopic?.title}</h2>
                    </div>
                    <button
                        onClick={handleMarkComplete}
                        disabled={completedTopics.has(activeTopic?.$id) || markingComplete}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${completedTopics.has(activeTopic?.$id)
